  ...
  <div class="app">
    ...
    <div class="main-area">
      ...
      <div class="user-panel">
        ...
        <UserPanel :users="users" :currentUser="currentUser" :activeUser="String(activeUser)" @selectUser="selectUser" />
        <!-- 公共聊天室 -->
    ...
    <div v-if="showUsers" class="mobile-user-panel">
      ...
      <div v-for="u in users" :key="u.id" class="mobile-user-item" @click="selectUser(u.id); showUsers = false">
        {{ u.username }}
      </div>
    </div>
  </div>
...
const currentUser = String(localStorage.getItem("userId"))
...
function selectUser(userId) {
  ...
  activeUser.value = String(userId)

  if (!conversations[activeUser.value]) {
    conversations[activeUser.value] = []
  }
}
...
function getActiveUsername() {
  if (activeUser.value === GLOBAL) return "公共聊天室"

  const user = users.value.find(
    u => String(u.id) === activeUser.value
  )
